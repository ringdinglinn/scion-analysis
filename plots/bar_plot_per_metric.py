...
EXTRA_DATASETS = [
    {
        "name": "BGP, CRVE 20%",
        "csv": "results/20251201_crve_20_complex.csv",
        "color": caida_conn.color,
    },
    {
        "name": "Expander",
        "csv": "results/Expanders_d16_complex.csv",
        "color": expander.color,
    },
]

# Optional single-sample datasets (no averaging)
ENABLE_SCION_TESTBED = False
ENABLE_SCION_TESTBED_REWIRED = False

SCION_TESTBED = {
    "name": "SCION testbed",
    "csv": "results/scion_testbed_complex.csv",
    "color": "orange",
}

SCION_TESTBED_REWIRED = {
    "name": "SCION testbed rewired",
    "csv": "results/scion_testbed_rewired_complex.csv",
    "color": "#dd7a09",
}
...
COLOR_MAIN = scion_conn.color
COLOR_MANUAL = scion_og.color

OUTPUT_DIR = "plots/imgs/bar_plots_combined"
DPI = 300
FILE_EXT = "pdf"
...
sample = load_single_sample(SCION_TESTBED, ENABLE_SCION_TESTBED)
if sample is not None:
    single_samples.append(sample)

sample = load_single_sample(SCION_TESTBED_REWIRED, ENABLE_SCION_TESTBED_REWIRED)
if sample is not None:
    single_samples.append(sample)
...
for fig_idx, layout in enumerate(FIGURES):
    ...
    filename = f"combined_{fig_idx}.{FILE_EXT}"
