...
from plots.graph_configs import caida
import re
...
for fig_idx, grid in enumerate(FIGURES):
    ...
    for r, row in enumerate(grid):
        for c, metric in enumerate(row):
            ...
            ax.plot(x, means, linewidth=1, color=caida.color, marker=".")
            ax.fill_between(x, lower, upper, color=caida.color, alpha=0.3)
